window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*
		* The host half of `dsh-openai-codex-auth` owns the device-code flow; the
		* card is a thin client: it writes an `action` ("login" | "logout") into the
		* `openai-codex-auth` settings namespace, then reflects the status fields
		* the host writes there (deviceCode, verificationUri, error) and
		* the configured state of the OAuth credential.
		*/

		/** Copy dictionary namespace owned by this plugin. */
		const NS = "settings.openai-codex-auth";
		/** Settings namespace the host uses as the login-status channel. */
		const LOGIN_NS = "openai-codex-auth";
		/** Credential reference holding the stored OAuth credential JSON. */
		const OAUTH_REF = "PI_OAUTH_OPENAI_CODEX";
		...
		/** Stable JSON equality for the small settings snapshot returned by DSH. */
		...
		/** Loads the login status + credential state and issues actions. */
		var LoginController = class {
			...
			async load({ background = false } = {}) {
				...
				try {
					const [settingsResponse, credentialsResponse] = await Promise.all([
						...
						this.api.credentials.describe({ refs: [OAUTH_REF] })
					]);
					if (!settingsResponse.result.ok) throw new Error(settingsResponse.result.error.message);
					if (!credentialsResponse.result.ok) throw new Error(credentialsResponse.result.error.message);
					if (generation !== this.generation) return;
					const view = settingsResponse.result.value.namespaces.find((candidate) => candidate.ns === LOGIN_NS);
					const credential = credentialsResponse.result.value.credentials[OAUTH_REF];
					const section = view === void 0 ? void 0 : view.value;
			...
			async setAction(action) {
				const response = await this.api.settings.mutate({
					ns: LOGIN_NS,
					ops: [{ op: "set", path: ["action"], value: action }]
				});
		...
		*/
		...
		function Loaded({ injected }) {
			...
			const usageUpdatedAt = section === void 0 ? void 0 : section["usageUpdatedAt"];
			// Local `requested` covers the gap between clicking login and the host's
			// first status write; once the host reports a flow state it takes over.
			const waiting = flowStatus === "waiting" || flowStatus === "starting" || requested && (flowStatus === void 0 || flowStatus === "idle");

			// Poll the host while a login is in flight.
			(0, react.useEffect)(() => {
				if (!waiting) return;
			...
			// Refresh once when a logged-in card has no data or its cached summary is stale.
			(0, react.useEffect)(() => {
				if (!loggedIn || !state.writable || state.status !== "ready") return;
				const staleReady = usageStatus === "ready" && (
					typeof usageUpdatedAt !== "number" || Date.now() - usageUpdatedAt >= 60000
				);
				const staleError = usageStatus === "error" && (
					typeof usageUpdatedAt !== "number" || Date.now() - usageUpdatedAt >= 60000
				);
				if (usageStatus !== void 0 && usageStatus !== "idle" && !staleReady && !staleError) return;
				void controller.setAction("refresh_usage").catch((error) => {
					setActionError(`${t("usageFailed")}: ${messageOf(error)}`);
				});
			}, [controller, loggedIn, state.status, state.writable, t, usageStatus, usageUpdatedAt]);

			const run = async (action) => {
				setBusy(true);
				setActionError(void 0);
				try {
					await controller.setAction(action);
					if (action === "login") setRequested(true);
				} catch (error) {
		...
		*/
		...
		/**
		* Register the ChatGPT login section once the `settings.section`
		* declaration is on the ledger, and keep its state fresh on pushed
		* settings/credentials invalidations.
		...
		*/
		function apply(ctx) {
			...
			const controller = new LoginController(connection.api);
			...
			ctx.effect(() => {
				...
				const disposers = [
					...
					ctx.get("remote").$on("credentials/updated", refresh),
					ctx.on("connection/reset", refresh)
				];
				return () => {
					for (const dispose of disposers) dispose();
				};
			}, "openai-codex-auth: pushed invalidations");
